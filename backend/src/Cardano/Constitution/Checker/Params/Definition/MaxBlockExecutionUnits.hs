  ...
  Collection @Integer
    ...
    [ Scalar
        ...
        , ("MEU-M-01", "*maxBlockExecutionUnits[memory]* **must not** be less than *maxTxExecutionUnits[memory]*") `ShouldSatisfy` \ctx val -> 
          case ctx.merged.byName.getIntegers "maxTxExecutionUnits" of 
            Nothing -> Unsatisfied "maxTxExecutionUnits not found"
            Just m -> case lookup "memory" m of
