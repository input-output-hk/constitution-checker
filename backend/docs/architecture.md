...
- `constitution-checker-backend` library
    ...
    - `Cardano.Constitution.Checker.Params.Definition` -  Concrete parameter definitions
      including their guardrails. Every parameter is defined separately in its own file in
      the `./src/Cardano/Constitution/Checker/Params/Definition` folder.
...
- Functional routes
  ...
  - `POST /parameters/proposal/by-url` - Same as `POST /parameters/proposal` but as input
    exepects an URL where the JSON object can be found
  - 
- Swagger utility
